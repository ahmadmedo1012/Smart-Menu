...
export default function AdminAuditLogsPage() {
	...
	if (error && logs.length === 0) {
		return (
			<div
				className="flex flex-col items-center justify-center py-20 gap-3 animate-fade-in"
				aria-live="assertive"
			>
	...
	return (
		<div className="space-y-6 animate-fade-in">
			...
			<div className="flex items-center gap-3">
				<div
					className="size-11 rounded-md bg-gradient-to-br from-orange to-orange/80 flex items-center justify-center shadow-lg"
					aria-hidden="true"
				>
				...
				<>
								...
								{logs.map((log) => {
									...
									return (
										<TableRow key={log.id} className="group">
											<TableCell>
												<Badge className={cn('font-medium', as.bg, as.color)}>{as.label}</Badge>
											</TableCell>
											...
											<TableCell
												className="text-xs text-muted-foreground hidden md:table-cell font-mono"
												dir="ltr"
											>
												{log.ip || '—'}
											</TableCell>
					...
					{expandedId !== null && (
						<div className="rounded-md border border-border/30 bg-muted/30 p-4 overflow-x-auto">
							<pre
								className="text-xs font-mono whitespace-pre-wrap break-words max-h-96 overflow-y-auto"
								dir="ltr"
							>
