...
export default function AdminQRPage() {
		...
		fetch('/api/auth/me')
			...
			.then((d) => {
				...
				if (
					role !== 'super_admin' &&
					role !== 'admin' &&
					!(permissions ?? []).includes('MANAGE_RESTAURANTS')
				) {
					setAccessDenied(true);
				}
			})
	...
	const menuUrl = selectedSlug ? `${window.location.origin}/menu/${selectedSlug}` : '';
		...
		<div className="space-y-6 animate-fade-in max-w-xl mx-auto">
			...
			{restaurants.length === 0 && (
				<div className="flex flex-col items-center justify-center py-16 text-center rounded-md bg-card/50 border border-border/30">
					...
					<p className="text-sm text-muted-foreground max-w-xs">
						لم يتم العثور على مطاعم. قم بإضافة مطعم أولاً لإنشاء رمز QR.
					</p>
				</div>
			)}
