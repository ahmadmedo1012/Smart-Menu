	...
	const handleOpen = (o: boolean) => {
		if (o && editing) {
			setForm({
				...
				showOnLanding: (editing as any).showOnLanding ?? false,
				featuredRank:
					(editing as any).featuredRank != null ? String((editing as any).featuredRank) : '',
			});
		} else if (o && !editing) {
			setForm({
				name: '',
			...
			if (editing) {
				...
				const res = await csrfFetch('/api/restaurants', {
					...
					body: JSON.stringify({
						...body,
						username: form.slug,
						password: `${form.slug}-${crypto.randomUUID().slice(0, 8)}`,
					}),
				});
				const json = await res.json();
			...
			<DialogContent className="max-w-lg rounded-md max-h-[85vh] overflow-y-auto">
						...
						<div>
							...
							<Input
								...
								onChange={(e) =>
									setForm({
										...form,
										slug: e.target.value.replace(/[^a-z0-9-]/g, '').toLowerCase(),
									})
								}
						...
						<label className="flex items-center gap-3 p-3 rounded-lg border border-border/20 cursor-pointer hover:bg-accent/30 transition-colors">
							...
							<div>
								...
								<p className="text-xs text-muted-foreground">
									ظهور المطعم في قائمة المطاعم المميزة بالصفحة الرئيسية
								</p>
							</div>
						</label>
					</div>
				...
				<div className="flex justify-end gap-2 mt-4">
					...
					<Button variant="orange" onClick={save} disabled={saving}>
						{saving ? 'جارٍ...' : editing ? 'تحديث' : 'إضافة'}
					</Button>
				</div>
			</DialogContent>
		</Dialog>
