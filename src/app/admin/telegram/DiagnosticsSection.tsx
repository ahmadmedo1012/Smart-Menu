...
export function DiagnosticsSection({
	...
	const handleDeleteApprover = async (id: number) => {
		try {
			const res = await csrfFetch(`/api/admin/telegram/approvers/${id}`, { method: 'DELETE' });
			const json = await res.json();
								...
								<div>
									...
									<div className="space-y-2">
										{diagnose.broadcastTargets.map((t) => (
											<div
												key={t.id}
												className="flex items-center justify-between p-2 rounded-lg bg-background/50 border border-border/20"
											>
