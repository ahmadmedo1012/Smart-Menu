...
export default function AdminTelegramPage() {
  ...
  const handleSave = async () => {
    if (!config.botToken.trim() || !config.chatId.trim()) { premiumToast("error", "يرجى إدخال رمز البوت ومعرف المحادثة"); return }
    setSaving(true)
    try {
    ...
    } catch (e: any) { premiumToast("error", e.message || "فشل حفظ الإعدادات") }
  ...
  const handleTest = async () => {
    ...
    } catch (e: any) { premiumToast("error", e.message || "فشل إرسال رسالة الاختبار") }
  ...
  const handleDiagnose = async () => {
    ...
    } catch (e: any) { premiumToast("error", e.message || "فشل التشخيص") }
  ...
  const handleAddTarget = async (label: string, chatId: string) => {
    ...
    } catch (e: any) { premiumToast("error", e.message || "فشل إضافة جهة الإرسال") }
  }

  const handleToggleTarget = async (id: number, isActive: boolean) => {
    try {
      const res = await csrfFetch(`/api/telegram/broadcast-targets/${id}`, {
    ...
    } catch (e: any) { premiumToast("error", e.message || "فشل تحديث الحالة") }
  }

  const handleDeleteTarget = async (id: number) => {
    try {
      const res = await csrfFetch(`/api/telegram/broadcast-targets/${id}`, { method: "DELETE" })
      const json = await res.json()
      if (!json.success) throw new Error(json.error || "فشل الحذف")
      setTargets((prev) => prev.filter((t) => t.id !== id))
      premiumToast("success", "تم حذف جهة الإرسال")
    } catch (e: any) { premiumToast("error", e.message || "فشل حذف جهة الإرسال") }
  }
