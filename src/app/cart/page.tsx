  ...
  if (items.length === 0 && !confirmed) {
    return (
      ...
      <div className="flex flex-col items-center justify-center min-h-[80vh] gap-4 px-4 text-center animate-fade-in">
        <div className="size-24 rounded-2xl bg-gradient-to-br from-gold/20 to-gold/10 flex items-center justify-center animate-float">
    ...
    <>
          ...
          <button
            ...
            className={cn(
              ...
              pickupType === opt.value
                ? "bg-gradient-to-r from-gold to-gold/80 text-white border-gold shadow-lg shadow-gold/20 scale-[1.02]"
                : "bg-card/50 border-border/30 hover:border-gold/20 hover:shadow-md",
            )}
          >
            {opt.icon === "inside" ? (
      ...
      <div className="space-y-4 mb-8">
        {items.map((item, idx) => (
          <div
            key={item.id}
            className={cn(
              "rounded-2xl bg-card/60 border border-border/30 p-4 transition-all duration-500 hover:border-gold/20 hover:shadow-lg hover:-translate-y-0.5",
          ...
          >
            <div className="flex items-center justify-between mb-3 gap-2">
              <div className="flex-1 min-w-0 flex items-center gap-3">
                <div className="size-10 rounded-xl bg-gradient-to-br from-gold/20 to-gold/10 flex items-center justify-center shrink-0">
              ...
              <div className="flex items-center gap-1.5">
                <button type="button" onClick={() => updateQuantity(item.id, item.quantity - 1)}
                  className="size-9 rounded-xl border border-border/30 flex items-center justify-center hover:bg-gold-muted hover:border-gold/30 transition-all active:scale-90">
            ...
            <input type="text" placeholder="ملاحظات للصنف..." value={item.notes}
              ...
              className="w-full h-9 rounded-xl border border-input bg-transparent px-3 text-sm outline-none transition-all focus-visible:border-gold focus-visible:ring-4 focus-visible:ring-gold/20" />
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl bg-card/60 border border-border/30 p-5 mb-6 animate-slide-up delay-300">
        ...
        <textarea placeholder="أي ملاحظات للطلب..." value={notes}
          ...
          className="w-full rounded-xl border border-input bg-transparent px-4 py-3 text-sm outline-none transition-all focus-visible:border-gold focus-visible:ring-4 focus-visible:ring-gold/20 resize-none" />
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            <input id="cart-name" type="text" dir="auto" placeholder="الاسم (اختياري)" maxLength={30}
              ...
              className="w-full h-11 rounded-xl border border-input bg-transparent px-4 text-sm outline-none transition-all focus-visible:border-gold focus-visible:ring-4 focus-visible:ring-gold/20" />
          </div>
          <div>
      ...
      <div className="rounded-2xl bg-gradient-to-r from-gold/5 to-gold/5 border border-gold/20 p-5 mb-8 animate-slide-up delay-400">
        <h3 className="text-sm font-bold mb-4 text-muted-foreground">ملخص الطلب</h3>
        <div className="space-y-2 mb-4">
          {items.map((item) => (
            <div key={item.id} className="flex justify-between text-sm group hover:bg-gold-muted/50 -mx-2 px-2 py-1 rounded-lg transition-colors">
        ...
        <div className="h-px bg-gradient-to-r from-transparent via-gold/30 to-transparent my-3" />
      ...
      <Button className="w-full h-14 text-base font-semibold rounded-2xl gap-2 shadow-lg shadow-gold/20 hover:shadow-xl hover:shadow-gold/30 transition-all duration-300"
      ...
      <Dialog open={showPreview} onOpenChange={setShowPreview}>
        <DialogContent className="rounded-3xl gap-0 p-0 overflow-hidden sm:max-w-md">
          <div className="bg-gradient-to-r from-gold to-gold/80 p-5 text-white">
            <DialogHeader>
          ...
          <div className="p-5 space-y-4">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-medium px-3 py-1 rounded-full bg-gold-muted text-gold/80 dark:text-gold">
                {pickupType === "inside" ? (
