...
export function AnalyticsTab({ advancedStats }: { advancedStats: AdvancedStats }) {
	const revenueSparkline = useMemo(
		() => advancedStats.revenue7d.map((r) => r.revenue),
		[advancedStats.revenue7d]
	);
	const ordersSparkline = useMemo(
		() => advancedStats.orders7d.map((o) => o.count),
		[advancedStats.orders7d]
	);
	const chartData = useMemo(
		() => advancedStats.revenue7d.map((d) => ({ label: d.date.slice(5), value: d.revenue })),
		[advancedStats.revenue7d]
	);
	const hourlyData = useMemo(
		() => advancedStats.hourlyDistribution.map((h) => ({ label: `${h.hour}:00`, value: h.count })),
		[advancedStats.hourlyDistribution]
	);
	const revenue7dTotal = useMemo(
		() => advancedStats.revenue7d.reduce((s, r) => s + r.revenue, 0),
		[advancedStats.revenue7d]
	);
	const orders7dTotal = useMemo(
		() => advancedStats.orders7d.reduce((s, o) => s + o.count, 0),
		[advancedStats.orders7d]
	);
	const aovAvg = useMemo(
		() =>
			Math.round(
				advancedStats.aovTrend.reduce((s, a) => s + a.aov, 0) /
					Math.max(advancedStats.aovTrend.length, 1)
			),
		[advancedStats.aovTrend]
	);
								...
								<div key={item.itemId} className="flex items-center justify-between py-1.5">
									...
									<div className="flex items-center gap-2">
										...
										{item.growth !== 0 && (
											...
											>
												{item.growth > 0 ? '↑' : '↓'}
												{toArabicNumber(Math.abs(item.growth))}%
											</span>
										)}
									</div>
								</div>
