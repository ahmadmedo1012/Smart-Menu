    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard
          ...
          value={advancedStats.revenue7d.reduce((s, r) => s + r.revenue, 0)}
          icon={DollarSign}
          iconBg="bg-success/10"
          iconColor="text-success"
          suffix=" د.ل"
          trend={advancedStats.growthPct}
          sparklineData={advancedStats.revenue7d.map(r => r.revenue)}
        />
        <KpiCard
          label="طلبات 7 أيام"
          value={advancedStats.orders7d.reduce((s, o) => s + o.count, 0)}
          icon={ShoppingCart}
          iconBg="bg-orange-muted"
          iconColor="text-orange"
          sparklineData={advancedStats.orders7d.map(o => o.count)}
        />
        <KpiCard
          label="متوسط قيمة الطلب"
          value={Math.round(advancedStats.aovTrend.reduce((s, a) => s + a.aov, 0) / Math.max(advancedStats.aovTrend.length, 1))}
          icon={TrendingUp}
      ...
      <div className="rounded-md bg-card/50 border border-border/30 p-5 shadow-sm">
        ...
        {advancedStats.revenue7d.length > 0 ? (
          <AreaChart
            data={advancedStats.revenue7d.map(d => ({ label: d.date.slice(5), value: d.revenue }))}
            height={200}
          />
        ) : (
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-md bg-card/50 border border-border/30 p-5 shadow-sm">
          ...
          {advancedStats.hourlyDistribution.length > 0 ? (
            <HorizontalBar
              data={advancedStats.hourlyDistribution.map(h => ({ label: `${h.hour}:00`, value: h.count }))}
              barHeight={20}
            />
          ) : (
