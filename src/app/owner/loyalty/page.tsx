...
const LoyaltySettings = dynamic(
	() =>
		import('@/components/loyalty/LoyaltySettings').then((m) => ({ default: m.LoyaltySettings })),
	{ ssr: false }
);
	...
	const statCards = [
		{
			...
			value: stats?.totalLoyaltyCards ?? 0,
			icon: Users,
					...
					<div className="flex flex-col items-center gap-3">
						...
						<div className="w-full flex gap-2">
							<Button
								...
								onClick={() =>
									window.open(
										`https://wa.me/?text=${encodeURIComponent(`ادعوك لتجربة المنيو الذكي\n${ownerRefUrl}`)}`,
										'_blank'
									)
								}
							>
			...
			<div className="grid gap-6 lg:grid-cols-2">
				<div className="card-premium relative overflow-hidden rounded-md bg-white/60 backdrop-blur-xl dark:bg-white/5 border border-white/30 dark:border-border/20">
					<div className="flex items-center justify-between border-b border-border/20 px-5 py-4">
						...
						<Badge
							variant="outline"
							className="text-[10px] border-white/20 bg-white/30 dark:bg-white/5"
						>
