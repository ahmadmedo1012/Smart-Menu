...
export default function OwnerMenuPage() {
	...
	const fetchCats = useCallback(async () => {
		...
		try {
			setLoading(true);
			setError(null);
			const r = await fetch(`/api/categories?restaurantId=${restaurantId}`);
			if (!r.ok) throw Error(`${r.status}`);
			const j = await r.json();
			const cats = j.events ?? j.data ?? j;
	...
	const fetchItems = useCallback(async (catId: number) => {
		try {
			const r = await fetch(`/api/items?categoryId=${catId}`);
			const j = await r.json();
			setCategories((p) =>
				p.map((c) =>
					c.id === catId ? { ...c, items: Array.isArray(j.data ?? j) ? (j.data ?? j) : [] } : c
				)
			);
		} catch {
			premiumToast('error', 'فشل تحميل الأصناف');
		}
	}, []);

	const toggleCat = (id: number) => {
		setExpandedCat((p) => (p === id ? null : id));
		if (expandedCat !== id) fetchItems(id);
	};

	const saveCat = async () => {
		if (!catForm.name.trim()) {
			premiumToast('error', 'يرجى إدخال الاسم');
			return;
		}
		try {
			const body = {
			...
			if (catEditing)
				await csrfFetch(`/api/categories/${catEditing.id}`, {
					method: 'PUT',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify(body),
				});
			else
				await csrfFetch('/api/categories', {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify(body),
				});
			premiumToast('save', catEditing ? 'تم التحديث' : 'تمت الإضافة');
			setCatDialog(false);
			fetchCats();
			setUsageKey((k) => k + 1);
		} catch {
	...
	const delTarget = async () => {
		...
		try {
			await csrfFetch(
				`/api/${deleteTarget.type === 'category' ? 'categories' : 'items'}/${deleteTarget.id}`,
				{ method: 'DELETE' }
			);
			premiumToast('trash', 'تم الحذف');
			setDeleteTarget(null);
			if (deleteTarget.type === 'category') fetchCats();
			else if (expandedCat) fetchItems(expandedCat);
			setUsageKey((k) => k + 1);
		} catch {
	...
	const toggleStatus = async (item: Item) => {
		...
		try {
			...
			premiumToast('save', ns === 'available' ? 'متوفر' : 'غير متوفر');
	...
	if (error)
		return (
			<div className="flex flex-col items-center justify-center py-20 gap-4 animate-fade-in">
				...
				<Button variant="outline" size="sm" onClick={() => fetchCats()} className="gap-1.5">
					إعادة المحاولة
				</Button>
			</div>
		);
			...
			{filteredCategories.length === 0 ? (
				<div className="flex flex-col items-center justify-center py-20 text-muted-foreground gap-4 animate-fade-in">
					...
					<p className="text-sm text-muted-foreground/60">
						{searchTerm ? 'جرب كلمات بحث أخرى' : 'أضف تصنيفاً جديداً لبدء بناء المنيو'}
					</p>
					{!searchTerm && (
						<Button
							variant="orange"
							onClick={() => {
								setCatEditing(null);
								setCatForm({ name: '', nameAr: '', icon: 'Package' });
								setCatDialog(true);
							}}
							className="gap-2"
						>
				...
				<div className="space-y-3">
					{filteredCategories.map((cat) => (
						<div
							key={cat.id}
							className="rounded-md border border-border/30 bg-card/50 overflow-hidden transition-all hover:border-orange/30 hover:shadow-md"
						>
							<div
							...
							>
								<div className="flex items-center gap-3">
									{cat.icon && cat.icon in CATEGORY_ICON_MAP ? (
										(() => {
											const I = CATEGORY_ICON_MAP[cat.icon];
											return <I className="size-6 text-primary" />;
										})()
									) : (
									...
									<div>
										...
										{cat.nameAr && (
											<span className="text-xs text-muted-foreground mr-2">{cat.nameAr}</span>
										)}
											...
											{cat.items.map((item) => (
												...
												>
													...
													<div className="flex-1 min-w-0">
														...
														{item.description && (
															<p className="text-xs text-muted-foreground/50 truncate max-w-md">
																{item.description}
															</p>
														)}
													</div>
													<div className="text-end shrink-0">
														...
														{item.discountedPrice && (
															<span className="text-xs text-muted-foreground/50 line-through block">
													...
													<div className="flex items-center gap-2">
														...
														<span
															className={cn(
																'text-xs w-14',
																item.status === 'available' ? 'text-success' : 'text-destructive'
															)}
														>
			...
			<Dialog open={catDialog} onOpenChange={setCatDialog}>
				<DialogContent className="rounded-md">
					...
					<div className="space-y-4">
						<div>
							...
							<input
								...
								className="w-full h-11 rounded-[4px] border border-border/30 px-4 text-sm outline-none mt-1.5 focus-visible:border-orange"
							/>
						</div>
						<div>
							...
							<input
								...
								className="w-full h-11 rounded-[4px] border border-border/30 px-4 text-sm outline-none mt-1.5 text-left"
								dir="ltr"
							/>
						</div>
						<div>
							...
							<div className="flex flex-wrap gap-2 mt-1.5">
								{CATEGORY_ICONS.map((iconName) => {
									const IconComp =
										iconName in CATEGORY_ICON_MAP ? CATEGORY_ICON_MAP[iconName] : null;
									...
									return (
										<button
											...
											className={cn(
												'size-10 rounded-[4px] flex items-center justify-center border transition-all',
												catForm.icon === iconName
													? 'border-orange bg-orange-muted dark:bg-orange-muted scale-110'
					...
					<div className="flex justify-end gap-2 mt-2">
						...
						<Button variant="orange" onClick={saveCat}>
							{catEditing ? 'تحديث' : 'إضافة'}
						</Button>
					</div>
				</DialogContent>
			</Dialog>
			...
			<Dialog open={deleteTarget !== null} onOpenChange={(o) => !o && setDeleteTarget(null)}>
				<DialogContent className="rounded-md">
					<DialogHeader>
						...
						<DialogDescription>
							هل أنت متأكد من حذف &ldquo;{deleteTarget?.name}&rdquo;؟ هذا الإجراء لا يمكن التراجع
							عنه.
						</DialogDescription>
					</DialogHeader>
					<div className="flex justify-end gap-2">
						...
						<Button variant="destructive" onClick={delTarget}>
							حذف
						</Button>
					</div>
				</DialogContent>
