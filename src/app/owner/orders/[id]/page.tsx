...
export default function OwnerOrderDetail({ params }: { params: Promise<{ id: string }> }) {
	...
	const updateStatus = async (status: string) => {
		...
		try {
			...
			premiumToast('error', 'فشل تحديث الحالة');
		} finally {
			setUpdating(false);
		}
	};
	...
	const copyAsWhatsApp = () => {
		...
		navigator.clipboard.writeText(text).then(() => {
			setCopied(true);
			premiumToast('copy', 'تم نسخ نص الطلب');
			...
			<div className="space-y-4 animate-fade-in max-w-3xl mx-auto">
				...
				<div className="h-40 rounded-md bg-card/50 border border-border/20 p-6 space-y-3 animate-pulse">
					...
					<div className="flex justify-between">
						{[1, 2, 3, 4].map((i) => (
							<div key={i} className="size-10 rounded-xl bg-muted/40 animate-pulse" />
						))}
					</div>
				</div>
							...
							<div key={s} className="flex-1 flex flex-col items-center">
								<button
									...
									className={cn(
										...
										isCurrent
											? 'bg-gradient-to-r from-orange to-orange/80 text-white border-orange shadow-lg shadow-orange/25 scale-110'
											: isActive
					...
					{order.status !== 'cancelled' && order.status !== 'completed' && (
						<>
							...
							<Button
								variant="outline"
								onClick={() => {
									if (window.confirm('هل أنت متأكد من إلغاء هذا الطلب؟')) updateStatus('cancelled');
								}}
					...
					<div className="space-y-3">
						...
						<div>
							...
							<a
								...
								className="font-medium text-primary hover:text-orange transition-colors flex items-center gap-1"
								dir="ltr"
							>
					...
					<div className="space-y-3">
						...
						<div className="flex justify-between font-bold text-lg">
							...
							<span className="tabular-nums text-primary">
								{toArabicNumber(order.total.toFixed(1))} د.ل
							</span>
						</div>
					</div>
				</div>
			...
			<div className="rounded-md bg-card/50 border border-border/30 overflow-hidden">
				...
				<div className="divide-y divide-border/10">
					{order.items.map((oi) => (
						...
						>
							<div className="flex items-center gap-3">
								<span className="size-7 rounded-lg bg-orange-muted dark:bg-orange-muted text-orange/80 dark:text-orange text-xs font-bold flex items-center justify-center">
