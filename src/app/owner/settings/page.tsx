...
export default function OwnerSettingsPage() {
		...
		Promise.all([
			...
			.then(([settingsData, plansData]) => {
				...
				if (r) {
					...
					setPickupTypes(
						r.pickupTypes ? r.pickupTypes.split(',') : ['inside', 'takeaway', 'delivery']
					);
				}
	...
	const save = async () => {
		setSubmitted(true);
		if (!form.name.trim()) {
			premiumToast('error', 'يرجى إدخال اسم المطعم');
			return;
		}
		if (pickupTypes.length === 0) {
			premiumToast('error', 'يرجى اختيار نوع طلب واحد على الأقل');
			return;
		}
		setSaving(true);
		try {
					...
					<div className="flex items-start justify-between gap-3">
						<div className="flex items-center gap-3">
							...
							<div>
								...
								{currentPlan?.price ? (
									<p className="text-xs text-muted-foreground">{currentPlan.price} د.ل/شهر</p>
								) : null}
							</div>
						</div>
				...
				<div className="flex items-center gap-5">
					...
					<div className="flex-1">
						<p className="text-xs text-muted-foreground mb-2">
							يظهر في رأس صفحة المنيو. يفضل صورة مربعة 512×512
						</p>
						<input
							ref={logoInputRef}
							type="file"
				...
				{gallery.length === 0 ? (
					<div className="text-center py-10">
						...
						<p className="text-[11px] text-muted-foreground/60 mt-0.5">
							أضف صوراً للمطعم لتظهر في صفحة المنيو
						</p>
					</div>
				) : (
					<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2.5">
						{gallery.map((url, i) => (
							<div
								key={i}
								className="group relative aspect-square rounded-xl overflow-hidden bg-muted/20 border border-border/20"
							>
								...
								<div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
									{i > 0 && (
										<button
											...
											className="size-10 rounded-full bg-background/70 text-foreground flex items-center justify-center hover:bg-background text-sm"
										>
											→
										</button>
									)}
			...
			<div className="rounded-md bg-card/40 border border-border/20 p-5">
				...
				<div className="space-y-2.5">
					{[
						...
						<label
							key={opt.value}
							className="flex items-center gap-3 p-3 rounded-md border border-border/20 cursor-pointer hover:bg-orange-muted/30 transition-colors"
						>
							<input
								type="checkbox"
								checked={pickupTypes.includes(opt.value)}
								onChange={(e) =>
									setPickupTypes(
										e.target.checked
											? [...pickupTypes, opt.value]
											: pickupTypes.filter((v) => v !== opt.value)
									)
								}
				...
				{pickupTypes.length === 0 && (
					<p className="text-xs text-destructive mt-2">يجب اختيار نوع طلب واحد على الأقل</p>
				)}
			</div>
				...
				<div className="space-y-3.5">
					<div>
						...
						<Input
							...
							className={cn(
								'h-10 rounded-xl mt-1 text-sm',
								submitted && !form.name.trim() && 'border-destructive ring-1 ring-destructive/30'
							)}
					...
					<div className="grid grid-cols-2 gap-3">
						<div>
							...
							<Input
								...
								className="h-10 rounded-xl mt-1 text-sm text-left"
							/>
						</div>
						<div>
					...
					<div className="grid grid-cols-2 gap-3">
						...
						<div>
							...
							<Input
								...
								placeholder="8:00 صباحاً - 12:00 منتصف الليل"
								className="h-10 rounded-xl mt-1 text-sm"
							/>
						</div>
					</div>
				...
				<div className="grid grid-cols-2 gap-3">
					...
					<div className="rounded-md bg-card/40 border border-border/20 p-4">
						...
						<p className="text-xl font-bold">
							{toArabicNumber(restaurant._count?.categories ?? 0)}
						</p>
					</div>
				</div>
			)}
