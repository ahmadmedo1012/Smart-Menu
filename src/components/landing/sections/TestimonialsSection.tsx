...
export default function TestimonialsSection() {
  ...
  return (
    <section className="relative py-20 sm:py-28 overflow-hidden" id="reviews">
      {/* Ambient glow pools */}
      ...
      <div className="relative z-10 max-w-[1220px] mx-auto px-4 sm:px-6">
        {/* Section header — minimal, no eyebrow */}
          ...
          <div className="relative w-full max-w-2xl">
            ...
            <AnimatePresence mode="wait" custom={dir}>
              <motion.div
                key={page}
                custom={dir}
                initial={{ opacity: 0, x: dir > 0 ? 80 : -80, rotateZ: dir > 0 ? 2 : -2 }}
                animate={{ opacity: 1, x: 0, rotateZ: 0 }}
                exit={{ opacity: 0, x: dir > 0 ? -80 : 80, rotateZ: dir > 0 ? -2 : 2 }}
                transition={{ duration: 0.5, ease: EASE }}
                className="relative rounded-2xl border border-border/50 bg-card p-8 sm:p-10 shadow-xl shadow-black/20"
              >
                {/* Floating quote icon */}
