...
const StickyMenuHeader = dynamic(
	() => import('./StickyMenuHeader').then((m) => ({ default: m.StickyMenuHeader })),
	{ ssr: false }
);
...
const LottieAnimation = dynamic(
	() => import('@/components/shared/LottieAnimation').then((m) => ({ default: m.LottieAnimation })),
	{ ssr: false }
);
const ShareButton = dynamic(
	() => import('@/components/shared/ShareButton').then((m) => ({ default: m.ShareButton })),
	{ ssr: false }
);
				...
				<div className="relative z-10 max-w-4xl mx-auto px-4 pt-24 pb-12 md:pt-28 md:pb-16 text-center">
					{restaurant.logo ? (
						<div className="relative mx-auto mb-6 group">
							...
							<div
								className="absolute -inset-2 rounded-full border border-orange/5 animate-hero-glow-pulse"
					...
					<h1 className="text-4xl md:text-6xl lg:text-7xl font-bold mb-3 text-balance leading-[1.35] bg-gradient-to-l from-orange via-orange/80 to-orange/40 bg-clip-text text-transparent drop-shadow-sm">
						{restaurant.name}
					</h1>
					...
					<div className="flex flex-wrap items-center justify-center gap-2.5 mt-4">
						...
						<a
							...
							target="_blank"
							className="inline-flex items-center gap-1.5 px-3.5 py-2.5 min-h-11 rounded-full text-xs font-medium glass-card hover:bg-orange-muted transition-all duration-300 backdrop-blur-md border border-white/5 dark:border-white/[0.04]"
						>
							<svg
								className="size-3.5"
								viewBox="0 0 24 24"
