          ...
          <div className={cn("p-6 space-y-4", !item.image && "pt-8")}>
            ...
            <div className="grid grid-cols-2 gap-2">
              <input value={customerName} onChange={e => setCustomerName(e.target.value)}
                ...
                className="h-11 rounded-xl border border-border/30 bg-card/50 px-4 text-sm outline-none focus-visible:border-orange" />
              <input value={customerPhone} onChange={e => setCustomerPhone(e.target.value)}
                placeholder="رقم الهاتف (اختياري)" maxLength={15} dir="ltr"
                className="h-11 rounded-xl border border-border/30 bg-card/50 px-4 text-sm outline-none focus-visible:border-orange text-left" />
            </div>

            {/* Quantity selector */}
            <div className="flex items-center justify-between rounded-2xl bg-orange-muted/30 border border-orange/15 p-4">
              <div>
              ...
              <div className="flex items-center gap-3">
                <button type="button" onClick={() => setQuantity(Math.max(1, quantity - 1))} disabled={quantity <= 1}
                  className="size-10 rounded-xl border border-border/40 flex items-center justify-center hover:bg-orange-muted hover:border-orange/30 disabled:opacity-30">
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
              ...
              rows={2} className="w-full rounded-xl border border-input bg-transparent px-4 py-3 text-sm outline-none transition-all focus-visible:border-orange focus-visible:ring-4 focus-visible:ring-orange/20 resize-none" />
