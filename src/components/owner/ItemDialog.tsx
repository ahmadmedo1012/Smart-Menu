...
export function ItemDialog({
	...
	const save = async () => {
		...
		if (!form.name.trim() || !form.price) {
			premiumToast('error', 'يرجى إدخال الاسم والسعر');
			return;
		}
		if (form.image && !IMAGE_URL_RE.test(form.image)) {
			premiumToast('error', 'رابط الصورة غير صالح');
			return;
		}
		setSaving(true);
		try {
			...
			const res = editing
				? await csrfFetch(`/api/items/${editing.id}`, {
						method: 'PUT',
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-lg rounded-md">
					...
					<div className="grid grid-cols-2 gap-3">
						...
						<div>
							...
							<Input
								...
								className="h-11 rounded-md mt-1.5 text-left"
								dir="ltr"
							/>
						</div>
					</div>
					<div className="grid grid-cols-2 gap-3">
						...
						<div>
							...
							<Textarea
								...
								className="mt-1.5 rounded-md text-left"
								rows={2}
								dir="ltr"
							/>
						</div>
					...
					<div>
						...
						<div className="flex gap-2 mt-1.5">
							<Input
								...
								className="h-11 rounded-md text-left flex-1"
								dir="ltr"
								placeholder="https://..."
								disabled={uploading}
							/>
							<label
								className="size-11 rounded-md border border-border/30 flex items-center justify-center hover:bg-accent cursor-pointer shrink-0"
						...
						{form.image && (
							<div className="mt-2 rounded-md overflow-hidden size-20 border border-border/30">
								<OptimizedImage src={form.image} alt="" className="size-full" skeleton={false} />
							</div>
						)}
					</div>
					<div>
				...
				<div className="flex justify-end gap-2 mt-2">
					...
					<Button variant="orange" onClick={save} disabled={saving || uploading}>
						{editing ? 'تحديث' : 'إضافة'}
					</Button>
				</div>
			</DialogContent>
