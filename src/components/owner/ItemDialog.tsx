    ...
    <Dialog open={open} onOpenChange={o => { if (o) openDialog(); onOpenChange(o); }}>
      <DialogContent className="max-w-lg rounded-2xl">
              ...
              {["available","unavailable"].map(s => (
                <button key={s} type="button" onClick={() => setForm({...form, status: s})}
                  className={cn("flex-1 py-2.5 rounded-xl text-sm font-medium border transition-all",
                    form.status === s ? s === "available" ? "bg-emerald-500/10 border-emerald-400/30 text-emerald-700" : "bg-red-500/10 border-red-400/30 text-red-700" : "border-border/30 hover:border-gold/30")}>
        ...
        <div className="flex justify-end gap-2 mt-2">
          ...
          <Button onClick={save} className="rounded-xl bg-gold hover:opacity-90 text-white">{editing ? "تحديث" : "إضافة"}</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
