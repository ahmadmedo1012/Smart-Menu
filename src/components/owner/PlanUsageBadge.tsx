...
export function PlanUsageBadge({ restaurantId }: { restaurantId: number }) {
	const [usage, setUsage] = useState<{ current: number; max: number; planName: string } | null>(
		null
	);
	...
	return (
		<div
			className={cn(
				...
				isAt
					? 'bg-red-50 dark:bg-red-950/20 border-red-200/30'
					: isNear
						? 'bg-orange/[0.06] dark:bg-orange/10 border-orange/20'
			...
			<div className="flex items-center justify-between mb-2">
				<div className="flex items-center gap-2 text-sm">
					...
					<span
						className={cn(
							'tabular-nums',
							isAt ? 'text-destructive font-bold' : isNear ? 'text-orange' : ''
						)}
					>
