		...
		result: {
			menuItem: {
				price: {
					needs: { price: true },
					compute(i) {
						return toNumber(i.price, 'menuItem.price');
					},
				},
				discountedPrice: {
					needs: { discountedPrice: true },
					compute(i) {
						return toNumberOrNull(i.discountedPrice, 'menuItem.discountedPrice');
					},
				},
			},
			order: {
				subtotal: {
					needs: { subtotal: true },
					compute(i) {
						return toNumber(i.subtotal, 'order.subtotal');
					},
				},
				discount: {
					needs: { discount: true },
					compute(i) {
						return toNumber(i.discount, 'order.discount');
					},
				},
				total: {
					needs: { total: true },
					compute(i) {
						return toNumber(i.total, 'order.total');
					},
				},
			},
			orderItem: {
				price: {
					needs: { price: true },
					compute(i) {
						return toNumber(i.price, 'orderItem.price');
					},
				},
			},
			subscriptionPlan: {
				price: {
					needs: { price: true },
					compute(i) {
						return toNumber(i.price, 'subscriptionPlan.price');
					},
				},
			},
			subscriptionPayment: {
				amount: {
					needs: { amount: true },
					compute(i) {
						return toNumber(i.amount, 'subscriptionPayment.amount');
					},
				},
			},
			loyaltyCard: {
				totalSpent: {
					...
					compute(i) {
						return toNumber(i.totalSpent, 'loyaltyCard.totalSpent');
					},
				},
			},
		},
