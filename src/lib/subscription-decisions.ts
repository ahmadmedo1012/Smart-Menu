...
async function notifyUserViaTelegram(chatId: string | null, text: string) {
  ...
  try {
    ...
    console.error("[telegram] notify user failed:", e);
  }
}
  ...
  try {
    ...
    if (userChatId) {
      notifyUserViaTelegram(String(userChatId),
        ...
        .catch((e) => console.error("[subscription] notify user failed:", e));
    }
  ...
  try {
    ...
    if (userChatId) {
      notifyUserViaTelegram(String(userChatId),
        ...
        .catch((e) => console.error("[subscription] cancel notify user failed:", e));
    }

    return { ok: true, action: "cancelled", paymentId: existing!.id };
  } catch (e: unknown) {
    console.error("[subscription-decisions] cancel error:", e);
