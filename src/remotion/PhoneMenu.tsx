...
function Screen({ screen, progress }: { screen: typeof SCREENS[number]; progress: number }) {
	...
	return (
		<div style={{ opacity, transform: `translateY(${y}px)`, display: "flex", flexDirection: "column", gap: 10, willChange: "transform" }}>
			<div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6, fontSize: 14, color: "oklch(0.5 0.01 0)" }}>
				<div style={{ flex: 1, height: 1, backgroundColor: "oklch(0.22 0.008 0)" }} />
				<span style={{ fontWeight: 500 }}>{screen.category}</span>
				<div style={{ flex: 1, height: 1, backgroundColor: "oklch(0.22 0.008 0)" }} />
			</div>
			{screen.items.map((item, i) => {
				...
				return (
					<div key={item.name} style={{
						...
						backgroundColor: item.color, fontSize: 16, opacity: itemOpacity, willChange: "opacity",
					}}>
						<span style={{ fontWeight: 500, color: "oklch(0.93 0.005 0)" }}>{item.name}</span>
						<span style={{ fontWeight: 700, color: "#f66d0f", fontSize: 15 }}>{item.price}</span>
					</div>
				)
			})}
		</div>
...
export const PhoneMenuVideo: React.FC = () => {
	...
	return (
		<div style={{
			...
			<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, fontSize: 16, color: "oklch(0.5 0.01 0)" }}>
				<span style={{ fontWeight: 600 }}>9:41</span>
				<div style={{ display: "flex", gap: 6 }}>
					<div style={{ width: 8, height: 8, borderRadius: "50%", backgroundColor: "#f66d0f" }} />
					<div style={{ width: 8, height: 8, borderRadius: "50%", backgroundColor: "oklch(0.5 0.01 0)" }} />
				</div>
			</div>

			<div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
				<div style={{ width: 32, height: 32, borderRadius: 6, backgroundColor: "#f66d0f", display: "flex", alignItems: "center", justifyContent: "center" }}>
					<svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth={2}>
						<path d="M3 12h2M21 12h-2M12 3v2M12 21v-2" strokeLinecap="round"/>
					</svg>
				</div>
				<span style={{ fontSize: 18, fontWeight: 700, color: "oklch(0.93 0.005 0)" }}>المنيو</span>
			</div>
			...
			<div style={{
				...
				gap: 6, padding: "12px 24px", borderRadius: 6, backgroundColor: "#f66d0f",
			...
			<div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 16 }}>
				{SCREENS.map((_, i) => (
					<div key={i} style={{
						width: i === currentScreen ? 24 : 8, height: 8,
						borderRadius: 4, backgroundColor: i === currentScreen ? "#f66d0f" : "oklch(0.5 0.01 0)",
						transition: "all 0.3s",
					}} />
				))}
			</div>
