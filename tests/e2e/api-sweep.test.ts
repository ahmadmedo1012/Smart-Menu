...
test.describe('Restaurants — GET /api/restaurants', () => {
	...
	test('returns success:true with restaurants array and total', async ({ request }) => {
		const json = await expectJson(await request.get(`${BASE}/api/restaurants`), 200);
		expect(json.success).toBe(true);
		expect(json.data as Record<string, unknown>).toHaveProperty('restaurants');
		expect(Array.isArray((json.data as Record<string, unknown>).restaurants)).toBe(true);
		expect(json.data as Record<string, unknown>).toHaveProperty('total');
		expect(typeof (json.data as Record<string, unknown>).total).toBe('number');
	});

	test('respects pageSize pagination param', async ({ request }) => {
		const json = await expectJson(await request.get(`${BASE}/api/restaurants?pageSize=3`), 200);
